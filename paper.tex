...
\title{plyranges: a grammar for transforming genomics data  }



\author{\Large Stuart Lee\vspace{0.05in} \newline\normalsize\emph{Monash University}   \and \Large Michael Lawrence\vspace{0.05in} \newline\normalsize\emph{Genentech}   \and \Large Di Cook\vspace{0.05in} \newline\normalsize\emph{Monash University}  }
...
\hypersetup{breaklinks=true,
            bookmarks=true,
            pdfauthor={Stuart Lee (Monash University) and Michael Lawrence (Genentech) and Di Cook (Monash University)},
             pdfkeywords = {Bioconductor, Grammar, Genomes, Data Analysis},  
            pdftitle={plyranges: a grammar for transforming genomics data},
            colorlinks=true,
            citecolor=blue,
            urlcolor=blue,
            linkcolor=magenta,
...
\textbf{\authorfont Stuart Lee} \hskip 15pt \emph{\small Monash University}   \par \textbf{\authorfont Michael Lawrence} \hskip 15pt \emph{\small Genentech}   \par \textbf{\authorfont Di Cook} \hskip 15pt \emph{\small Monash University}   
...
language and API design: cognitive consistency, cohesion, expressiveness
and endomorphism \cite{Green1996-qg}. To varying degrees, these
principles also underly the design of dplyr and the Bioconductor
infrastructure.
...
basic data.frame and dplyr tibble lack any notion of genomic ranges and
so could not easily support our genomic grammar, with its specific verbs
for range-oriented data manipulation. Another example of cognitive
consistency is how plyranges is insensitive to direction/strand by
default when, e.g., detecting overlaps. GenomicRanges has the opposite
behavior. We believe that defaulting to purely spatial overlap is most
intuitive to most users.

Like dplyr, plyranges verbs are functional: they are free of side
effects and return their result. This enables chaining of verbs through
syntax like the forward pipe operator (\texttt{\%\textgreater{}\%}, read
as ``then'') of the magrittr package \cite{R-magrittr}. This has syntax
has a direct cognitive mapping to natural language and the intuitive
notion of pipelines. The low-level object-oriented APIs of Bioconductor
tend to manipulate data via subreplacement functions, like
\texttt{start(gr)\ \textless{}-\ x}. These ultimately produce the side
effect of replacing a symbol mapping in the current environment and thus
are not amenable to so-called fluent syntax.
...
\texttt{resize()} function from the GenomicRanges package has a
...
the original GenomicRanges API. In other cases, the plyranges API
increases verbosity for the sake of clarity and cohesion. Explicitly
calling \texttt{anchor()} can require more typing, but the code is
easier to comprehend. Another example is the set of routines for
importing genomic annotations, including
\texttt{read\textbackslash{}\_gff()},
\texttt{read\textbackslash{}\_bed()}, and \texttt{read\_bam()}. Compared
to the generic \texttt{import()} in rtracklayer, the explicit
...
name, otherwise strand is ignored. The GenomicRanges API does not make
this distinction explicit in its function naming, instead relying on a
...
The Bioconductor package \texttt{AnnotationHub} \cite{R-ahub} can be
used to search for BigWig files from ChIP-Seq experiments from the Human
Epigenome Roadmap project \cite{Roadmap-Epigenomics-Consortium2015-pr}.
Here we focus on assays for primary T CD8+ memory cells from peripheral
blood. Using \texttt{plyranges} we will read the BigWig file
corresponding to the H3 lysine 27 trimethylation (H3K27Me3) methylation
mark over chromosome 10.
...
reduce individual runs to ranges representing the islands of coverage,
...
bins of fixed width 10000bp we apply \texttt{tile()} function from
\texttt{GenomicRanges} to the genomic coordinates (with \texttt{unlist}
to return a GRanges object)
...
\StringTok{  }\KeywordTok{tile}\NormalTok{(}\DataTypeTok{width =}\NormalTok{ 10000L) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{unlist}\NormalTok{(}\DataTypeTok{use.names =} \OtherTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Next we only read in alginments that overlap the genomic locations we
are interested in and select the query seqeunce. Note that the reading
...
Finally, we use \texttt{group\_by\_overlaps()} with \texttt{summarize()}
to compute the total number of reads and averge GC content within each
genomic window.
...
We can then compute the per-chromosome transition-transversion ratio
using the \texttt{group\_by} and \texttt{summarize} pattern (figure
\ref{fig:titv-viz}).
...
grammar for performing genomic arithemtic, aggregation, restriction and
...
by the Bioconductor ecosytem and the R language.
