...
data structures that naturally represent genomic intervals.
...
call returns the same type of object as its input.  

Then the last paragraph (already pretty close) states that:

    we aim to simplify the interface to the most fundamental Ranges data structures, which we must reuse for the reasons stated above,
    it is possible to align the Ranges data structures with the tabular notions of the dplyr API
    so we will extend dplyr to incorporate genomic notions

We have created a DSL called `plyranges` that uses the existing
`IRanges` and `GenomicRanges` packages in Bioconductor as a back-end. Our API 
still has the speed and efficiency of these packages but
with a consistent interface. The API also extends the grammar elements in `dplyr` 
for performing genomic specific manipulations such as finding overlapping 
regions or nearest neighbour regions between genomic intervals. 
The _plyranges_ DSL is specifically designed to enable fast interactive 
analysis of _Ranges_ objects but can also be used for scripting 
