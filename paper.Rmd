...
sequences. In the Bioconductor/R packages `IRanges` and `GenomicRanges` these 
representations have been implemented as a suite of data structures called
_Ranges_ @Lawrence2013-wg. These data structures cover many common data types 
encountered in bioinformatics analyses - a gene can be represented with its 
coordinates, along with its identifier and the identifiers of its exons; or
an RNA-seq experiment may be represented as sets of genes with a matching count 
column.

The Bioconductor infrastructure for computing with genomic ranges are highly 
effecient and powerful, however the application programming interface (API) for 
...
embedded in the unix environment ^[other ideas to mention GROK @Ovaska2013-gd]. 
...
of tidy data @Wickham2014-jc. The tidy data pattern is useful because it allows 
...
this abstraction: it is a rectanglular table corresponding to a single biological 
context. Each row contains a single observation and each column a variable about
that observation. 
...
implements and extends the grammar defined by the R package `dplyr` @Wickham2017-dplyr. 
The grammar provides a consistent way of interacting with and analysing genomic 
data via methods for constructing, grouping, mutating, filtering, and 
summarising _Ranges_ and an algebra for reasoning about actions on _Ranges_
...
use of non-standard evaluation in R via the _rlang_ package. Simply, this
means that compuations are performed and evaluated in the context of the 
Ranges objects, which emphasises the interactive nature of the API. This has
...
The _plyranges_ API provides the methods `as_granges()` and `as_iranges()`
for constructing Ranges from tabular data structures, such as the _data.frame_ in
...
convienence methods for annotating or extracting annotations from Genomic Ranges
objects with the `set_genome_info()` and `get_genome_info()` functions.

There is a consistent framework for reading and writing 
files from and to common genomic data formats, using the `rtracklayer` package
as a backend. The methods are implemented in the `read_/write_` family of 
functions, currently _plyranges_ can read and write BAM, BED, BEDPE, 
narrowPeaks, GFF/GTF, WIG and BigWig files.

As an example, we use the Bioconductor package AnnotationHub to search for 
...
methylation mark over chromosome 10. First we gather the BigWig file
and extract its annotation information and filter it to chromosome 10.

```{r load-bw, cache = TRUE}
# E048 corresponds to CD8+ T cells from peripheral blood
library(plyranges)
library(AnnotationHub)
q <-query(AnnotationHub(), c("EpigenomeRoadMap", "E048"))
...
10. We then add annotation information to ensure we know we are working
with the hg19 genome build.
...
object, hence it returns a DataFrame object.
...
The `reduce_ranges()` operation could be used to find coverage peaks 
accross chromosome 10. We can manually set a threshold to restrict genomic
regions to have a coverage score of greater than 8, and
then merge those regions and compute the maximum coverage over all the regions
that were reduced. 
...
the above operation requires users to understand what an run length encoding
object and views object means and perform additional manipulations to return
the result back to a _Ranges_.
...
in particular section: Trait-associated variants enrich in tissue-specific marks
