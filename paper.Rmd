...
of the genome space. Additional values can be bound to the bare _IntegerRanges_ or _GenomicRanges_ and form metadata columns.
...
and design principles of `dplyr`: fluency, consistency, cohesion and endomorphism. 

To promote fluency we have designed every function call to perform an
action on a _Ranges_ object and where possible
functions have few arguments. Each function is named after a verb and is designed
...
vocabulary of several verbs, rather than having to memorise functions (that
are named as nouns) with many arguments (which is required in the existing Bioconductor packages). Workflows can be composed by chaining verbs together into
'sentences' via the forward pipe operator,`%>%` (exported from the R package `magrittr` @R-magrittr). Overall, this allows users to write human readable code
because workflows describe what the code is doing rather than how its doing it.
