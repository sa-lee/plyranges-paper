...
[ML: why not get the seqinfo from the BAM itself?]

We reuse the genome information from our BigWig file, as the
...
[ML: seems like we should have a `select_seqlevels()` or something, so
     that we could do `select_seqlevels(-chrM)`.. somehow we have to
     explain to the reader why we are doing all of this seqlevel munging.]
...
To generate bins of width 10000bp we use the `tile` function from 
`GenomicRanges`:
...
[ML: seems like a great opportunity to point out the deferred reading]

Finally, we use `group_by_overlaps` with `summarize` to compute the
total number of reads within each window.

[ML: there was a typo in the code: have you actually run this?]

```{r, eval = FALSE}
alignments_summary <- bins %>%
  group_by_overlaps(alignments) %>%
  summarize(n_reads = n())
```
...
[ ML: there should be some quick summary paragraph here ]
