...
In order to provide a compatible API with _dplyr_, _plyranges_ makes use
of non-standard evaluation in R via the _rlang_ package. 
<!--- need to mention something about how this could be detrimental --->
...
for constructing Ranges from tabular data structures, such as data.frames in
base R. These methods use non-standard evaluation so columns in a data.frame
can be modified before a Ranges object is created. The API also has
...
blood. We can then read the BigWig file  corresponding to the H3K27Me3 
...
a 5000 nt region centered around the maximum position. 
...
the max peak on chromosome 10 (visualised in figure ()).

```{r peak_region, fig.align = "center", fig.cap = "Visualisation of peak coverage score accross 5000nt region of chromosome 10"}
...
we aim to use it for the visualisation of genomic ranges.
