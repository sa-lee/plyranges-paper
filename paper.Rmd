...
generally in the \\texttt{plyranges} DSL for both overlap and nearest neighbour
...
\\textit{y} ranges. \\textbf{C} For the left join \\textbf{left} join all 
\\textit{x} ranges are returned regardless of whether they are overlapped by 
\\textit{y}. In this case the resulting third range  of the join would have 
missing values on metadata columns that came from \\textit{y}.}
\label{fig:olaps-fig}
\end{figure}
...
than the equivalent `GenomicRanges` code (figure \ref{code-fig}).
...
construct the workflows we show the data output intermittantly to assist
