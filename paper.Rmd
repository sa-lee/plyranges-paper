...
To varying degrees, these principles also underly the
...
The Bioconductor package `AnnotationHub` \cite{R-ahub}  can be used to search for 
BigWig files from ChIP-Seq experiments from the Human Epigenome Roadmap project 
\cite{Roadmap-Epigenomics-Consortium2015-pr}. Here we focus on assays for primary
...
bins of fixed width 10000bp we apply `tile()` function from `GenomicRanges`
to the genomic coordinates (with `unlist` to return a GRanges object)
```{r bins}
...
we are interested in and select the query seqeunce. Note that
...
total number of reads and averge GC content within each genomic window.
...
We can then compute the per-chromosome transition-transversion ratio using the
...
the Bioconductor ecosytem and the R language.
...
`knitr` \cite{R-knitr} and the figures were made with `ggbio` \cite{R-ggbio}. All code
