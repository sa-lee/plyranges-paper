...
to get the range for chromosome 10. 

First, we gather the BigWig file and extract its annotation information and 
filter it to chromosome 10. This range will be used as  a filter when reading
the file.
...
10. We also add the genome build information to the resulting ranges.
...
via the `group_by_overlaps()` operator.  We bin and count reads from a
H3K27Me3 ChIP-seq experiment by the Human Epigenome Roadmap
Consortium. The bins have a fixed width of 10000bp.
...
bins of width 10000bp we apply `tile` function from `GenomicRanges`
...
In this case using `read_bam()` in conjunction with `get_genome_info()`
only loads the genome information and nothing else.

Next we only read in alginments that overlap the genomic locations
we are interested in. The reading is deferred until after the filter is 
constructed: that is only alignments that pass the filter are loaded into memory.
...
Finally, we use `group_by_overlaps` with `summarize` to compute the
...
We compute the per-chromosome transition-transversion ratio using the
`group_by` and `summarize` pattern (figure \ref{fig:titv-viz}).
...
implement a grammar for performing genomic arithemtic, aggregation, restriction 
and merging. Our examples show that `plyranges` code is succinct and human
readable.
...
integrates well with the grammar of graphics [cite ggplot2].  To
...
The `plyranges` package is available on the Bioconductor project
website [https://bioconductor.org](https://bioconductor.org) or can be
accessed via
Github
